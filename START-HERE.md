...
Finality, Temporal Issuance, or Possibility to Capability. It records 186
...
- one scoped `HC-DU-042` endogenous-record mechanism tournament and
  ...
  common knowledge, total order, and action safety remain separate. Only
  `N5-SCF-P3`, provenance-preserving metastable-to-Byzantine hardening, is
  executable; later positions remain conditional and `N5-RS-P2` is deferred
...
`SELECTION_OR_FORMATION_OBSTRUCTION`. `N5-SCF-P1/2` are complete as
`HC-DU-047/048`: downstream protocols cannot recover a target absent from the
...
ancestry without identifying physical source binding. Only `N5-SCF-P3`,
provenance-preserving metastable-to-Byzantine hardening, is executable;
...
| **3 — Physical record formation and selection** | Derive or delimit record instruments, interfaces, archives, and robustness from independently motivated physics. | Consume `HC-DU-044/045/046/047/048`: the host does not select completion, epoch, archive routing, or access; downstream layers cannot mint a target absent from the joint input; gossip can pool source-formed synergy and a signed DAG preserves only declared provenance. Use `N5-SCF-P3` to test whether hardening preserves or compresses those receipts; do not fit another detector, code, or archive. |
...
| **6 — Time, geometry, fields, and physical reconstruction** | Recover effective temporal, geometric, field, and particle structure with uniqueness and refinement controls. | `HC-DU-046` closes the first host; `HC-DU-047/048` separate joint-source synergy, pooling, declared provenance, knowledge, and downstream no-minting. `N5-SCF-P3` is the sole next object; the physical collision occurs only after the distributed/complexity/cryptographic layers are typed. |
...
7. For current central execution, read the
   ...
   `N5-SCF-P1/2` are complete as `HC-DU-047/048`. Only `N5-SCF-P3` is
   executable.
