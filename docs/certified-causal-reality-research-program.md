...
**Executed optical-clock evidence successor — `HC-DU-039D`.** In the
published trapped-ion-clock proposal, mean frequency shifts remain
reproducible by a fixed semiclassical average proper time. More strongly,
every two-level single-time dephasing factor \(\Gamma\), including reduced
visibility loss generated by a quantum motional label, has a classical
random-proper-time representation because the convex hull of phases is the
unit disk.

For an independently frozen finite history set \(\mathcal H\), conditioned
coherent recombination can nevertheless produce

\[
K_m=\sum_{h\in\mathcal H}c_{m,h}V_h
\]

with \(K_m^\dagger K_m=p_mI\) and
\(K_m/\sqrt{p_m}\not\propto V_h\) for every \(h\). The rank-one Choi ray then
lies outside the classical specified-history cone. The exact two-history
control returns
...
This is a history-relative nonclassical channel certificate, not a universal
classical exclusion or source attribution. If proper-time and matched
coherent-control contributions enter only through
\(\theta=\theta_\tau+\theta_\chi\), every output-only sensitivity row is
parallel to \((1,1)\), leaving exact local null \((1,-1)\). Full tomography
cannot distinguish identical channels. A source-selective intervention row
not parallel to \((1,1)\) is the minimum local parameter repair; equivalent
proper-time and mass-energy interpretations of one Hamiltonian remain
operationally indistinguishable.

The exact primary-source-pinned probe passes `36/36`. It establishes that
history nonclassicality, physical source attribution, and ontology are
separate reconstruction problems. It reports no observed proper-time effect,
new law, new physics, hardware result, or paper promotion. See
...
6. HC-DU-039 cross-platform invariance
   ...
   [COMPLETED EXACT KNOWN-MATHEMATICS SPECIALIZATION]
   HC-DU-039D optical-clock evidence hierarchy, specified-history Choi
   certificate and residual mechanism-attribution null
   [COMPLETED PRIMARY-SOURCE-PINNED KNOWN-MATHEMATICS SPECIALIZATION;
   SOURCE-SELECTIVE INTERVENTION CONTRACT OPEN]
        |
...
HC-DU-039D PROPER-TIME HISTORY CERTIFICATION AND SOURCE-ATTRIBUTION AXES SEPARATED
EVERY TWO-LEVEL SINGLE-TIME DEPHASING FACTOR CLASSICALLY RANDOM-TIME SIMULABLE
CONDITIONED COHERENT RECOMBINATION CAN LEAVE ONE FROZEN CLASSICAL-HISTORY SET
OUTPUT-ONLY PROPER-TIME / COHERENT-CONTROL NULL (1,-1) SURVIVES COMPLETE TOMOGRAPHY
SOURCE-SELECTIVE INTERVENTION, NOT ANOTHER OUTPUT OBSERVABLE, IS THE MINIMUM LOCAL RANK REPAIR
