...
| Event ontologies: Eventum, ETH, and Kent | Eventum constructs a nondemolition classical output history inside a supplied input--output model; ETH selects an actuality algebra by the center of a state's centralizer inside a supplied future-algebra co-filtration and adds a stochastic collapse law; Kent constructs one Lorentzian beable history from sampled asymptotic data | A formed output, an algebraically selected actuality algebra, and a sampled global completion are different objects. `HC-DU-091` proves that ETH's rule is star-isomorphism-covariant but not restriction/co-filtration-natural: one symmetric \(M_2(\mathbb C)\) antecedent admits incompatible \(X\)- and \(Z\)-event refinements. Once either atomic event family and time are fixed, the collapse step is the matched Lüders instrument. None of the three selects the complete provenance, access, certification, and regional-finality interface or presently isolates empirical excess | Stop event-to-record composition. Reopen only with a physical derivation of the future-algebra co-filtration that survives its physical equivalences and time/restriction choices, plus a no-refit consequence beyond the matched instrument ([HC-DU-090](../explorations/event-formation-antecedent-tournament-eventum-eth-kent-2026-07-28.md), [HC-DU-091](../explorations/eth-actuality-algebra-naturality-restriction-counterexample-and-instrument-boundary-2026-07-28.md)) |
