...
| Physical compactness versus finite certificate | `HC-DU-095` finite-reduces a compact target-separating capability surface. `HC-DU-096` shows how a fixed-domain higher-regularity bound can supply compactness only in a weaker target-continuous topology. Same-norm energy balls remain noncompact; causal cones can agree while curvature oscillates; reversible evolution transports rather than selects compactness | Derive one physically selected gauge/topology, regularity and noncollapse class closed in the held-out target topology, then construct a joint bounded QFT-realizable separating packet acquired below half its margin | Calling an energy bound compact, using weak convergence for a stronger target, crediting field evolution with selecting initial data, or treating one continuum trace as finite data ([HC-DU-095](../explorations/finite-target-resolution-certification-from-compact-physical-capability-surfaces-2026-07-28.md), [HC-DU-096](../explorations/physical-regularity-gap-compactness-and-geometric-target-topology-boundary-2026-07-28.md)) |
