...
| Event ontologies: Eventum, ETH, and Kent | Eventum constructs a nondemolition classical output history inside a supplied input--output model; ETH selects an actuality algebra by the center of a state's centralizer inside a supplied future-algebra co-filtration and adds a stochastic collapse law; Kent constructs one Lorentzian beable history from sampled asymptotic data | A formed output, an algebraically selected actuality algebra, and a sampled global completion are different objects. Even ETH's generating event partition can remain nonunique outside an atomic/minimal convention. None of the three currently selects the complete provenance, access, certification, and regional-finality interface or isolates empirical excess over its matched standard-quantum account | Test the ETH actuality-algebra quotient and its generating families for naturality under every physically admissible representation, future-algebra net, restriction, and time coarse-graining before attempting event-to-record composition; use Eventum as the supplied-channel control and Kent as the global-disclosure control ([HC-DU-090](../explorations/event-formation-antecedent-tournament-eventum-eth-kent-2026-07-28.md)) |
