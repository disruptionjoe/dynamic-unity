...
no-go's (check the tool's frame before globalizing an "impossible"). So: engage them in the open
exploration, let them sharpen the construction, and at the banking gate note honestly which a
...
1. **Block-universe absorber** (TaF). *Objection:* apparent passage shouldn't be promoted to
   ...
   (something crosses), not a static relabeling — i.e. win the D-FORK below. Beatable, not a wall.

2. **The D-FORK / Gödelian-regime question** (TI). *Objection:* record-accretion is genuine
   becoming only if the source is self-generating (Gödelian); in a finite-type source the same
   roll is fixed-source disclosure. *Frame:* the FTS regime (only). *Way through:* argue/pin that
   DU's source action sits in the Gödelian regime for its case — the general bit is non-computable,
   but a specific construction can carry evidence (N-curvature signature, growth witnesses). This
   is the sharpest challenge and the most interesting to win.

3. **Record-change ≠ finality** (continuity-ledger). *Type clarity, not a no-go:* keep
   record-accretion and finality separately typed; not every increment of N is a crossing.
   Respect it; it costs nothing and prevents a cheap error.

4. **N-wire-crossing** (TI). *Type clarity:* TI's N is a novelty *rate*; DU's N is an accreted
   *count* (`Λ~1/√N`). Different objects, same letter — keep them typed apart. A discipline, not
   an obstacle.

5. **Observer-gradient falsifier** (TaF). *Objection:* an observer-centred DE story predicts an
   observer-centred cosmological gradient, which observation excludes. *Way through:* keep the
   mechanism from implying such a gradient, or show why it doesn't. Cheap to check; genuinely
   useful as a sanity test.

6. **Type-Distance / "count of source activity" trap** (continuity-ledger). *Clarity:* if N is
